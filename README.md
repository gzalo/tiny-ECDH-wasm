# tiny-ECDH-wasm
WASM port of [tiny-ECDH-c](https://github.com/kokke/tiny-ECDH-c), so it can be used in browser JS.
...
Test:
 - Run `python3 -m http.server` and then go to [http://localhost:8000/ecdh_example.html](http://localhost:8000/ecdh_example.html). This is required as **ecdh_example.html needs to run in an HTTP server (because wasm files can't be loaded locally)**
